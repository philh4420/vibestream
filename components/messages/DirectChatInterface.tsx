...
export const DirectChatInterface: React.FC<DirectChatInterfaceProps> = ({ chatId, currentUser, allUsers, onBack, addToast, chatData }) => {
  ...
  const handleCall = (type: 'voice' | 'video') => {
    const featureName = type === 'voice' ? 'Neural Voice Uplink' : 'Holo-Video Sync';
    addToast(`${featureName} v3.0 Coming Soon`, "info");
  };
      ...
      <div className="px-6 md:px-8 py-4 md:py-6 border-b flex items-center justify-between backdrop-blur-3xl sticky top-0 z-20 bg-white/70 border-slate-100/50">
        <div className="flex items-center gap-4 md:gap-5">
          ...
          <div className="relative group">
            <img src={otherParticipant?.avatarUrl} className="w-12 h-12 md:w-14 md:h-14 border shadow-sm rounded-2xl" alt="" />
            {targetUserFull?.presenceStatus === 'Online' && <div className="absolute inset-0 rounded-2xl border-2 border-emerald-500 animate-ping opacity-20" />}
          </div>
          <div>
            <h3 className="font-black text-xl md:text-2xl uppercase tracking-tighter italic leading-none text-slate-950">{otherParticipant?.displayName}</h3>
        ...
        <div className="flex items-center gap-2 md:gap-3">
            <div className="hidden sm:flex bg-slate-100/80 p-1 rounded-[1.8rem] gap-1 shadow-inner border border-slate-100">
               <button onClick={() => handleCall('voice')} className="p-3 bg-white hover:bg-indigo-50 text-indigo-600 rounded-2xl transition-all shadow-sm flex items-center justify-center active:scale-90">
               ...
               <button onClick={() => handleCall('video')} className="p-3 bg-white hover:bg-emerald-50 text-emerald-600 rounded-2xl transition-all shadow-sm flex items-center justify-center active:scale-90">
            ...
            <button onClick={() => setTerminationTarget({ id: chatId, label: otherParticipant?.displayName || 'Link' })} className="p-3 md:p-4 bg-white/50 text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white transition-all shadow-sm active:scale-90 border border-slate-100">
              ...
              <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]`}>
                {!isMe && showAvatar && (
                  <div className="flex items-center gap-2 mb-2 ml-1">
                      ...
                      <span className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest font-mono italic">{otherParticipant?.displayName}</span>
                  </div>
                )}
      ...
      <div className="p-4 md:p-8 border-t bg-white/80 backdrop-blur-md border-slate-100/50 relative z-30">
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2 md:gap-4 max-w-5xl mx-auto items-end">
          <div className="flex gap-1.5 md:gap-2 pb-1.5 shrink-0">
            <button type="button" onClick={() => fileInputRef.current?.click()} className={`p-2.5 md:p-3 rounded-2xl transition-all active:scale-90 border ${selectedFile ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-white hover:border-indigo-100 hover:text-indigo-500 shadow-sm'}`}>
            ...
            <button type="button" onClick={() => { setIsGiphyPickerOpen(!isGiphyPickerOpen); setIsEmojiPickerOpen(false); }} className={`p-2.5 md:p-3 rounded-2xl transition-all active:scale-90 border ${isGiphyPickerOpen ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-white hover:border-indigo-100 hover:text-indigo-500 shadow-sm'}`}>
              <span className="text-[10px] font-black font-mono">GIF</span>
            </button>
            <button type="button" onClick={() => { setIsEmojiPickerOpen(!isEmojiPickerOpen); setIsGiphyPickerOpen(false); }} className={`p-2.5 md:p-3 rounded-2xl transition-all active:scale-90 border ${isEmojiPickerOpen ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-indigo-100 hover:text-indigo-500 shadow-sm'}`}>
          ...
          <div className="flex-1 relative">
            <input ref={inputRef} type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Establish broadcast sequence..." className="w-full bg-slate-100/80 border border-slate-200 rounded-[2.2rem] pl-6 pr-6 py-4 md:py-5 text-sm font-bold focus:ring-4 focus:ring-indigo-500/5 focus:bg-white focus:border-indigo-500 transition-all outline-none italic placeholder:text-slate-400 shadow-inner" />
          </div>
          
          <button disabled={(!newMessage.trim() && !selectedFile && !selectedGif) || isSending} className="h-[52px] md:h-[58px] px-6 md:px-8 bg-slate-950 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-black transition-all active:scale-95 disabled:opacity-30 italic shadow-xl group flex items-center justify-center shrink-0">
            {isSending ? (
