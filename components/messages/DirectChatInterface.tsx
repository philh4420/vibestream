...
export const DirectChatInterface: React.FC<DirectChatInterfaceProps> = ({ chatId, currentUser, allUsers, onBack, addToast, chatData }) => {
  ...
  const handleCall = (type: 'voice' | 'video') => {
    // In a real implementation, this would trigger the WebRTC overlay
    // For now, we simulate a system message or toast
    addToast(`Initiating ${type === 'voice' ? 'Audio' : 'Visual'} Uplink...`, "success");
  };
      ...
      <div className="absolute top-4 left-4 right-4 z-20">
        <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-[2rem] p-3 shadow-sm flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 pr-1">
             <button onClick={() => handleCall('voice')} className="w-10 h-10 bg-slate-50 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-2xl flex items-center justify-center transition-all active:scale-90">
             ...
             <button onClick={() => handleCall('video')} className="w-10 h-10 bg-slate-50 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-2xl flex items-center justify-center transition-all active:scale-90">
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path d="M15.75 10.5l4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25z" /></svg>
             </button>
             <button onClick={() => setTerminationTarget({ id: chatId, label: otherParticipant?.displayName || 'Link' })} className="w-10 h-10 bg-slate-50 hover:bg-rose-50 text-slate-400 hover:text-rose-500 rounded-2xl flex items-center justify-center transition-all active:scale-90">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar scroll-container px-4 md:px-8 pt-28 pb-32 space-y-6">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full opacity-30 text-center">
            <div className="w-20 h-20 bg-indigo-900/10 rounded-[2.5rem] flex items-center justify-center mb-6 text-indigo-900 scale-110"><ICONS.Messages /></div>
            <h3 className="text-xl font-black uppercase tracking-widest italic text-slate-900">Link_Established</h3>
            <p className="text-[10px] font-mono mt-2 font-bold text-slate-500 uppercase tracking-widest">Encrypted frequency ready for transmission.</p>
          </div>
        )}
              ...
              <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]`}>
                ...
                {!isMe && showAvatar && (
                  <div className="flex items-center gap-2 mb-2 ml-1">
                      <img src={otherParticipant?.avatarUrl} className="w-6 h-6 rounded-lg object-cover border border-slate-100 shadow-sm" alt="" />
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest font-mono">{otherParticipant?.displayName}</span>
                  </div>
                )}
        ...
        <div className="max-w-4xl mx-auto bg-white/90 backdrop-blur-2xl rounded-[2.5rem] p-2 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] border border-white/50 flex flex-col gap-2">
          ...
          {mediaPreview && (
            <div className="px-4 py-2 flex items-center gap-4 animate-in slide-in-from-bottom-2">
              <div className="relative group">
                ...
                <button onClick={clearMedia} className="absolute -top-2 -right-2 bg-slate-900 text-white rounded-full p-1 shadow-md scale-90 hover:scale-100 transition-all">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-600 truncate max-w-[150px]">{selectedFile?.name || 'GIF_FRAGMENT'}</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSendMessage} className="flex items-center gap-2 pl-2">
            ...
            <button 
              ...
              className="w-14 h-14 bg-slate-950 text-white rounded-[2rem] flex items-center justify-center transition-all active:scale-90 disabled:opacity-20 hover:bg-black shadow-lg"
            >
              {isSending ? (
