...
export const NeuralLinkOverlay: React.FC<NeuralLinkOverlayProps> = ({ session, userData, onEnd }) => {
  ...
  // Stable Audio Assets for UK Node (Avoiding 403 blocks)
  ...
  useEffect(() => {
    // Initialising reliable audio signals (CDN-hosted)
    ...
    const setupMedia = async () => {
      try {
        // ENFORCED MODALITY: Never request camera for voice-only links
  ...
  const handleDecline = async () => {
    if (!db) return;
    try { await updateDoc(doc(db, 'calls', session.id), { status: 'rejected' }); } finally { cleanup(); }
  };
  ...
  // FIX: Proper avatar mapping based on real-time peer data provided by the handshake session
  ...
  return (
    <div className="fixed inset-0 z-[3500] bg-[#020617] flex flex-col items-center justify-center p-4 md:p-10 selection:bg-indigo-500 overflow-hidden font-sans">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute top-0 left-0 w-full h-full transition-all duration-1000 ${callStatus === 'ringing' ? 'bg-[radial-gradient(circle_at_50%_50%,rgba(225,29,72,0.2),transparent_60%)]' : 'bg-[radial-gradient(circle_at_50%_50%,rgba(16,185,129,0.1),transparent_60%)]'}`} />
      ...
      <div className="relative z-10 w-full max-w-5xl h-full flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-700">
           ...
           {callStatus !== 'connected' && (
             <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 md:gap-10 p-6">
                <div className="relative">
                  <div className={`absolute inset-0 blur-3xl animate-pulse scale-150 ${session.type === 'video' ? 'bg-indigo-500/20' : 'bg-emerald-500/20'}`} />
                  <img 
                    src={targetAvatar} 
                ...
                <div className="text-center">
                  <h2 className="text-3xl md:text-5xl font-black text-white italic tracking-tighter uppercase mb-2 md:mb-4">
                    {isCaller ? (session.type === 'video' ? 'Syncing Video Link...' : 'Calling Node...') : session.callerName}
                  </h2>
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <p className="text-[10px] md:text-[12px] font-black text-indigo-400 uppercase tracking-[0.8em] font-mono animate-pulse">
                      {isIncoming ? 'INCOMING_HANDSHAKE' : `ESTABLISHING_${session.type.toUpperCase()}_MODALITY`}
                    </p>
                  </div>
                </div>
             </div>
        ...
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-6 md:gap-12 bg-white/5 backdrop-blur-3xl border border-white/10 p-5 md:p-8 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl">
             {isIncoming ? (
               <>
                 <button onClick={handleDecline} className="w-16 h-16 md:w-24 md:h-24 bg-rose-600 text-white rounded-2xl md:rounded-[2rem] flex items-center justify-center shadow-2xl hover:bg-rose-500 transition-all active:scale-90"><svg className="w-7 h-7 md:w-10 md:h-10 rotate-[135deg]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg></button>
                 <button onClick={handleAccept} className="w-20 h-20 md:w-32 md:h-32 bg-emerald-600 text-white rounded-[1.8rem] md:rounded-[2.5rem] flex items-center justify-center shadow-[0_0_50px_rgba(16,185,129,0.4)] hover:bg-emerald-500 transition-all active:scale-95 animate-bounce"><svg className="w-10 h-10 md:w-14 md:h-14" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg></button>
               </>
             ) : (
               <>
                 <button onClick={() => setIsMuted(!isMuted)} className={`w-14 h-14 md:w-20 md:h-20 rounded-2xl md:rounded-3xl flex items-center justify-center transition-all active:scale-90 border-2 ${isMuted ? 'bg-white text-slate-950 border-white' : 'bg-white/5 text-white border-white/10 hover:bg-white/10'}`}><svg className="w-6 h-6 md:w-8 md:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z" /></svg></button>
                 <button onClick={handleTerminate} className="w-16 h-16 md:w-28 md:h-28 bg-rose-600 text-white rounded-2xl md:rounded-[2.2rem] flex items-center justify-center shadow-2xl hover:bg-rose-500 transition-all active:scale-95 group"><svg className="w-8 h-8 md:w-12 md:h-12 rotate-[135deg] group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3.5"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg></button>
                 <button disabled={session.type === 'voice'} onClick={() => setIsVideoOff(!isVideoOff)} className={`w-14 h-14 md:w-20 md:h-20 rounded-2xl md:rounded-3xl flex items-center justify-center transition-all active:scale-90 border-2 ${session.type === 'voice' ? 'opacity-20 cursor-not-allowed' : (isVideoOff ? 'bg-white text-slate-950 border-white' : 'bg-white/5 text-white border-white/10 hover:bg-white/10')}`}><svg className="w-6 h-6 md:w-8 md:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M15.75 10.5l4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 002.25-2.25v-9a2.25 2.25 0 00-2.25-2.25h-9A2.25 2.25 0 002.25 7.5v9a2.25 2.25 0 002.25 2.25z" /></svg></button>
               </>
             )}
          </div>
          {callStatus === 'connected' && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 px-6 py-2.5 rounded-full backdrop-blur-md">
              <span className="text-[11px] font-black font-mono text-emerald-400 tracking-[0.4em] uppercase">{formatTime(duration)}</span>
            </div>
          )}
        </div>
      </div>
