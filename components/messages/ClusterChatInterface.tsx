      ...
      <div className="px-6 md:px-8 py-4 md:py-6 border-b flex items-center justify-between backdrop-blur-3xl sticky top-0 z-20 bg-white/70 border-slate-100/50">
        <div className="flex items-center gap-4 md:gap-5">
          ...
          <div className="relative group">
            <img src={chatData.clusterAvatar} className="w-12 h-12 md:w-14 md:h-14 border shadow-sm rounded-[1.4rem]" alt="" />
          </div>
          <div>
            <h3 className="font-black text-xl md:text-2xl uppercase tracking-tighter italic leading-none text-slate-950">{chatData.clusterName}</h3>
            <p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest font-mono text-indigo-500 mt-1 md:mt-2">{chatData.participants.length} Active Nodes</p>
          </div>
        </div>
        
        <div className="flex items-center gap-2 md:gap-3">
            {chatData.clusterAdmin === currentUser.id && (
                <button onClick={() => setTerminationTarget({ id: chatId, label: chatData.clusterName || 'Cluster' })} className="p-3 md:p-4 bg-white/50 text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white transition-all shadow-sm active:scale-90 border border-slate-100">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar scroll-container p-6 md:p-8 space-y-6 md:space-y-8 relative">
        {messages.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center opacity-10 pointer-events-none p-12 text-center">
            <div className="w-20 h-20 md:w-24 md:h-24 bg-slate-900 rounded-[3rem] mb-6 flex items-center justify-center text-white scale-125"><ICONS.Clusters /></div>
            <h3 className="text-lg md:text-xl font-black uppercase tracking-widest italic">Cluster_Synched</h3>
            <p className="text-[8px] md:text-[9px] font-mono mt-2">AWAITING_GROUP_BURST</p>
          </div>
        )}
        {messages.map((msg, idx) => {
          ...
          // DYNAMIC SENDER LOOKUP PER MESSAGE
              ...
              <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]`}>
                {!isMe && showAvatar && (
                  <div className="flex items-center gap-2 mb-2 ml-1">
                      <img src={senderDetails.avatarUrl} className="w-5 h-5 rounded-lg object-cover border border-slate-100" alt="" />
