...
export const MessagesPage: React.FC<MessagesPageProps> = ({ currentUser, locale, addToast, weather }) => {
  ...
  const isDarkAtmosphere = weather?.condition?.toLowerCase().includes('rain') || weather?.condition?.toLowerCase().includes('storm');
  ...
  return (
    <div className="flex h-[calc(100vh-var(--header-h)-var(--bottom-nav-h)-2rem)] md:h-[calc(100vh-var(--header-h)-4rem)] -mx-4 sm:-mx-6 md:-mx-10 lg:-mx-14 bg-white md:rounded-[3rem] overflow-hidden shadow-heavy relative border border-slate-100 animate-in fade-in duration-500">
      ...
      {/* 2. CHAT VIEWPORT WITH ATMOSPHERIC THEME */}
      <div className={`${view === 'list' ? 'hidden md:flex' : 'flex'} flex-1 flex-col min-w-0 bg-[#fdfdfe] relative`}>
        <AtmosphericBackground weather={weather}>
          {activeChat ? (
            <>
              {/* Thread Header */}
              <div className={`px-8 py-6 border-b flex items-center justify-between backdrop-blur-3xl sticky top-0 z-20 ${isDarkAtmosphere ? 'bg-black/20 border-white/5' : 'bg-white/60 border-slate-50'}`}>
                <div className="flex items-center gap-5">
                  <button onClick={() => setView('list')} className={`md:hidden p-3 ${isDarkAtmosphere ? 'text-white/40' : 'text-slate-400'}`}><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M15 19l-7-7 7-7" /></svg></button>
                  <div className="relative">
                    <img src={otherParticipant?.avatarUrl} className="w-14 h-14 rounded-2xl border border-white/10 shadow-sm" alt="" />
                    <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-white rounded-full shadow-lg" />
                  </div>
                  <div>
                    <h3 className={`font-black text-xl uppercase tracking-tighter italic leading-none ${isDarkAtmosphere ? 'text-white' : 'text-slate-950'}`}>{otherParticipant?.displayName}</h3>
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2`}>
                      <div className={`max-w-[80%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                        <div className={`p-5 rounded-[2rem] text-sm font-bold shadow-sm backdrop-blur-xl ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : isDarkAtmosphere ? 'bg-white/5 text-white border border-white/10 rounded-tl-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-50 shadow-float'}`}>
                          {msg.text}
                        </div>
                        <span className={`text-[8px] font-black font-mono mt-2 px-2 uppercase tracking-widest ${isDarkAtmosphere ? 'text-white/20' : 'text-slate-300'}`}>
              ...
              {/* Input Hub */}
              <div className={`p-8 border-t backdrop-blur-3xl ${isDarkAtmosphere ? 'bg-black/20 border-white/5' : 'bg-white border-slate-50'}`}>
                <form onSubmit={handleSendMessage} className="flex gap-4 items-center">
                  <button type="button" className={`p-5 rounded-3xl transition-all ${isDarkAtmosphere ? 'bg-white/5 text-white/20 hover:bg-white/10' : 'bg-slate-50 text-slate-300 hover:bg-slate-100'}`}><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path d="M12 4.5v15m7.5-7.5h-15" /></svg></button>
                  <div className={`flex-1 border rounded-[2.5rem] px-8 py-4 shadow-inner flex items-center transition-all ${isDarkAtmosphere ? 'bg-white/5 border-white/10 focus-within:border-indigo-500' : 'bg-slate-50 border-slate-100 focus-within:border-indigo-100'}`}>
                    <input 
                      type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)}
                      placeholder="Broadcast your frequency..."
                      className={`w-full bg-transparent text-sm font-bold outline-none italic ${isDarkAtmosphere ? 'text-white placeholder:text-white/10' : 'text-slate-900 placeholder:text-slate-300'}`}
                    />
                  </div>
                  <button 
                    disabled={!newMessage.trim() || isSending}
                    className={`px-10 py-5 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.4em] transition-all active:scale-95 disabled:opacity-50 italic ${newMessage.trim() ? 'bg-indigo-600 text-white shadow-xl' : isDarkAtmosphere ? 'bg-white/5 text-white/20' : 'bg-slate-100 text-slate-300'}`}
                  >
                    TRANSMIT
                  </button>
                </form>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
              <div className="relative mb-12 opacity-10">
                 <div className={`scale-[3] ${isDarkAtmosphere ? 'text-white' : 'text-slate-900'}`}><ICONS.Search /></div>
              </div>
              <p className={`text-[12px] font-black uppercase tracking-[0.6em] font-mono leading-loose italic ${isDarkAtmosphere ? 'text-white/20' : 'text-slate-300'}`}>Establish_Initial_Ping</p>
            </div>
          )}
