...
export const SettingsOverlay: React.FC<SettingsOverlayProps> = ({ userData, onClose, onLogout, addToast }) => {
  ...
  const [settings, setSettings] = useState<UserSettings>(userData.settings ? { ...DEFAULT_SETTINGS, ...userData.settings } : DEFAULT_SETTINGS);
  ...
  const addHiddenWord = (e: React.FormEvent) => {
    ...
    setSettings(prev => ({
      ...prev,
      safety: { ...prev.safety, hiddenWords: [...prev.safety.hiddenWords, hiddenWordInput.trim()] }
    }));
    setHiddenWordInput('');
  };
  ...
  const clearCache = () => {
    ...
    addToast("Local Fragments Purged", "success");
  };
  ...
  const TabButton = ({ id, label, icon: Icon }: { id: SettingTab, label: string, icon: any }) => (
    <button
      ...
      className={`w-full flex items-center gap-4 px-5 py-4 rounded-[1.2rem] transition-all duration-300 group ${
        activeTab === id 
  ...
  const Toggle = ({ label, checked, onChange, description }: { label: string, checked: boolean, onChange: () => void, description?: string }) => (
    <div className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 hover:border-indigo-100 transition-all group">
      <div className="flex flex-col">
        <span className="text-xs font-bold text-slate-900 group-hover:text-indigo-900 transition-colors">{label}</span>
        {description && <span className="text-[9px] font-medium text-slate-400 mt-0.5">{description}</span>}
      </div>
      <button 
        onClick={onChange}
        className={`w-11 h-6 rounded-full p-1 transition-all duration-300 relative ${checked ? 'bg-indigo-600' : 'bg-slate-200'}`}
      >
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-0 md:p-6 animate-in fade-in duration-300">
      ...
      <div className="relative bg-[#fcfcfd] w-full max-w-6xl h-full md:h-[90vh] md:rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-500 border border-white/20">
        ...
        {/* Navigation Sidebar */}
        <div className="w-full md:w-72 lg:w-80 bg-slate-50/80 border-b md:border-b-0 md:border-r border-slate-200/60 p-4 md:p-8 flex flex-row md:flex-col gap-2 shrink-0 overflow-x-auto md:overflow-y-auto no-scrollbar">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#fcfcfd] relative">
          ...
          {/* Header Mobile */}
          <div className="md:hidden p-6 border-b border-slate-100 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
             <div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                 <div className="p-8 rounded-[2.5rem] bg-slate-900 text-white shadow-xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex items-center gap-6">
                      <div className="relative">
                        ...
                        {userData.verifiedHuman && <div className="absolute -bottom-2 -right-2 bg-emerald-500 p-1.5 rounded-xl text-white"><ICONS.Verified /></div>}
                      </div>
                      <div>
                         ...
                         <p className="text-sm font-mono text-slate-400 mb-4">ID: {userData.id}</p>
                         <button className="text-[9px] font-black uppercase tracking-widest text-indigo-300 hover:text-white transition-colors flex items-center gap-2">
                 ...
                 <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       <button className="p-6 rounded-[2rem] border border-slate-200 hover:border-indigo-200 bg-white hover:shadow-lg transition-all text-left group">
                 ...
                 <div className="pt-8 border-t border-slate-100">
                    ...
                    <div className="p-6 rounded-[2rem] bg-rose-50/50 border border-rose-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                       ...
                       <button className="px-6 py-3 bg-white border border-rose-200 text-rose-600 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all shadow-sm">
                          Deactivate
                       </button>
                    </div>
                 </div>
                    ...
                    <div className="space-y-3">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <button 
                            ...
                            className={`p-6 rounded-[2rem] border text-left transition-all ${settings.privacy.profileVisibility === 'public' ? 'bg-indigo-600 text-white border-indigo-600 shadow-xl ring-4 ring-indigo-100' : 'bg-white border-slate-200 text-slate-500 hover:border-indigo-200'}`}
                          >
                          ...
                          <button 
                            ...
                            className={`p-6 rounded-[2rem] border text-left transition-all ${settings.privacy.profileVisibility === 'private' ? 'bg-slate-900 text-white border-slate-900 shadow-xl ring-4 ring-slate-100' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-800'}`}
                          >
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-8">
                       {['system', 'light', 'dark'].map((theme) => (
                         <button
                           ...
                           className={`py-6 rounded-[2rem] text-[10px] font-black uppercase tracking-widest border transition-all ${
                    ...
                    <div className="grid grid-cols-3 gap-3">
                       {(['standard', 'strict', 'relaxed'] as const).map((level) => (
                         <button
                           ...
                           className={`py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest border transition-all ${
                 ...
                 <div className="pt-6 border-t border-slate-100">
                    ...
                    <form onSubmit={addHiddenWord} className="flex gap-2 mb-4">
                       <input 
                         ...
                         className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-100"
                       />
                       <button type="submit" className="px-6 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all">Add</button>
                    </form>
                    <div className="flex flex-wrap gap-2">
                       {settings.safety.hiddenWords.map(word => (
                         <button 
                           ...
                           className="px-3 py-1.5 bg-slate-100 rounded-lg text-[10px] font-bold text-slate-600 hover:bg-rose-100 hover:text-rose-600 flex items-center gap-2 transition-colors"
                         >
            ...
            {activeTab === 'data' && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="flex flex-col gap-3">
                       {[
                         ...
                         <button
                           ...
                           className={`p-4 rounded-[1.8rem] border text-left transition-all ${
                             settings.dataUsage.mediaQuality === opt.id 
                               ? 'bg-indigo-50 border-indigo-600 relative overflow-hidden' 
                 ...
                 <div className="space-y-4 pt-6 border-t border-slate-100">
                    ...
                    <button 
                      onClick={clearCache}
                      className="w-full flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 hover:border-rose-200 group transition-all"
                    >
          ...
          {/* Persistent Footer */}
          <div className="p-6 md:p-8 border-t border-slate-100 bg-white/80 backdrop-blur-xl flex justify-end gap-4 shrink-0 absolute bottom-0 left-0 right-0 z-10 md:relative">
             <button 
               onClick={onClose}
             ...
             <button 
               ...
               disabled={isSaving}
               className="px-10 py-4 rounded-[1.5rem] bg-indigo-600 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
             >
