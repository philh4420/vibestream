...
export const PostCard: React.FC<PostCardProps> = ({ 
  ...
  // COSMETIC INTEGRATION: BORDERS AND FILTERS
      ...
      <div className="p-6 md:p-9">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className={`relative shrink-0 w-14 h-14 rounded-[1.6rem] ${borderClass}`}>
            ...
            <div>
                <h3 className="font-black text-slate-900 dark:text-white text-base uppercase italic tracking-tight">{post.authorName}</h3>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex gap-4">
            <button 
              ...
              className={`flex items-center gap-3 h-12 px-6 rounded-2xl transition-all border ${post.isLiked ? 'bg-rose-50 dark:bg-rose-900/30 border-rose-100 text-rose-600' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 hover:bg-white'}`}
            >
            ...
            <button 
                ...
                className={`flex items-center gap-3 h-12 px-6 rounded-2xl transition-all border ${showComments ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}
            >
          ...
          <button 
            ...
            className={`w-12 h-12 flex items-center justify-center rounded-2xl transition-all ${isBookmarked ? 'text-indigo-600 bg-indigo-50' : 'text-slate-300 hover:text-slate-900'}`}
          >
            <ICONS.Saved />
          </button>
        </div>
