...
export const DataVaultPage: React.FC<DataVaultPageProps> = ({ currentUser, locale, addToast, onViewPost }) => {
  ...
  return (
    <div className="w-full max-w-[2400px] mx-auto pb-32 animate-in fade-in duration-700">
      ...
      {/* 1. VAULT HEADER */}
      <div className="relative rounded-[3rem] bg-[#0f172a] p-10 md:p-12 text-white shadow-2xl border border-slate-800 overflow-hidden mb-8 group selection:bg-cyan-500 selection:text-black">
      ...
      <div className="sticky top-[calc(var(--header-h)+1rem)] z-30 mb-8 px-2 md:px-0">
         <div className="bg-white/90 backdrop-blur-xl border border-white/60 p-2 rounded-[2.5rem] shadow-[0_20px_60px_-20px_rgba(0,0,0,0.1)] flex flex-col md:flex-row items-center justify-between gap-3">
            ...
            <div className="flex bg-slate-100/80 p-1 rounded-[2rem] w-full md:w-auto">
               {[
                 ...
                 <button
                   ...
                   className={`flex-1 md:flex-none px-8 py-3 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all duration-300 relative ${activeFilter === tab.id ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-700'}`}
                 >
                   {tab.label}
                 </button>
               ))}
            ...
            <div className="relative w-full md:w-96 group">
               <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-cyan-600 transition-colors scale-90">
                 <ICONS.Search />
               </div>
               <input 
                 type="text" 
                 ...
                 className="w-full bg-slate-50 border border-slate-200 rounded-[2rem] pl-14 pr-6 py-3.5 text-xs font-bold outline-none focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-200 focus:bg-white transition-all placeholder:text-slate-400 shadow-inner text-slate-700"
               />
            </div>
         </div>
      </div>
      ...
      <div className="min-h-[400px]">
         {loading ? (
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
             {filteredPosts.map((post, idx) => (
               <div 
                 ...
                 className="group bg-white rounded-[2.5rem] p-4 border border-slate-100 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] hover:border-cyan-200 transition-all duration-500 cursor-pointer relative flex flex-col h-full overflow-hidden hover:-translate-y-1"
                 style={{ animationDelay: `${idx * 50}ms` }}
               >
                  {/* Visual Header */}
                  <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden bg-slate-100 mb-4 border border-slate-50">
                     {post.media && post.media.length > 0 ? (
                       ...
                       <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center bg-slate-50/50">
                          ...
                          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest font-mono">Text_Log</p>
                       </div>
                     )}
                  ...
                  <div className="px-2 flex-1 flex flex-col">
                     <div className="flex items-center gap-2 mb-2">
                        <img src={post.authorAvatar} className="w-5 h-5 rounded-lg object-cover bg-slate-100" alt="" />
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest font-mono truncate">{post.authorName}</span>
                     </div>
                     <p className="text-xs font-bold text-slate-800 line-clamp-2 leading-relaxed mb-4 group-hover:text-cyan-700 transition-colors">
                       "{post.content || 'Encrypted visual artifact.'}"
                     </p>
                     
                     <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                        <span className="text-[8px] font-mono font-bold text-slate-400">{post.timestamp?.toDate ? post.timestamp.toDate().toLocaleDateString(locale) : 'Unknown Date'}</span>
                        <button 
                          onClick={(e) => handleUnsave(e, post.id)}
                          className="text-[8px] font-black text-rose-400 hover:text-rose-600 uppercase tracking-widest hover:underline decoration-rose-200 transition-all opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 duration-300"
                        >
                          Purge
                        </button>
                     </div>
           ...
           <div className="py-40 flex flex-col items-center justify-center text-center opacity-50 bg-slate-50/50 rounded-[4rem] border-2 border-dashed border-slate-200">
              ...
              <h3 className="text-2xl font-black uppercase tracking-widest italic text-slate-900">Vault_Empty</h3>
