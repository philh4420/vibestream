...
export const VerifiedNodesPage: React.FC<VerifiedNodesPageProps> = ({ users, onViewProfile }) => {
  ...
  return (
    <div className="w-full max-w-[2400px] mx-auto space-y-8 pb-24 animate-in fade-in duration-700">
      ...
      {/* 1. Verified Header */}
      <div className="relative overflow-hidden rounded-[3rem] bg-slate-950 p-10 md:p-12 text-white shadow-2xl border border-white/10">
         <div className="absolute top-0 right-0 w-96 h-96 bg-amber-500/10 blur-[120px] rounded-full translate-x-1/3 -translate-y-1/3" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
         {verifiedUsers.map((user, idx) => {
           ...
           return (
             <div 
               key={user.id} 
               className="group bg-white border border-slate-100 rounded-[3rem] p-6 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.08)] hover:border-amber-200 transition-all duration-500 relative flex flex-col animate-in fade-in slide-in-from-bottom-4"
             ...
             >
                   ...
                   <div className="flex items-center gap-4 cursor-pointer" onClick={() => onViewProfile(user)}>
                      <div className="relative">
                         <div className="p-1 bg-gradient-to-tr from-amber-200 to-yellow-500 rounded-[1.8rem] shadow-lg group-hover:scale-105 transition-transform duration-500">
                            <img src={user.avatarUrl} className="w-16 h-16 rounded-[1.6rem] object-cover border-2 border-white bg-slate-50" alt="" />
                         </div>
                         <div className="absolute -bottom-2 -right-2 bg-slate-950 text-amber-400 p-1.5 rounded-xl border-4 border-white shadow-md scale-90">
                            <ICONS.Verified />
                         </div>
                      </div>
                      <div>
                         <h3 className="text-lg font-black text-slate-900 uppercase italic tracking-tighter truncate leading-none mb-1 group-hover:text-amber-600 transition-colors">{user.displayName}</h3>
                         <div className="flex items-center gap-2">
                            <span className="text-[9px] font-bold text-slate-400 font-mono tracking-wider">@{user.username}</span>
                            {user.presenceStatus === 'Online' && <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />}
                         </div>
                      </div>
                   </div>
                   
                   <button onClick={() => onViewProfile(user)} className="p-3 bg-slate-50 hover:bg-white border border-slate-100 hover:border-slate-200 rounded-2xl text-slate-300 hover:text-slate-900 transition-all active:scale-90">
                ...
                <div className="mb-8 px-1">
                   <p className="text-[11px] text-slate-500 font-medium leading-relaxed line-clamp-2 h-9 italic">
                     "{user.bio || 'Verified entity. Establishing secure grid presence.'}"
                   </p>
                   
                   <div className="mt-4 flex items-center gap-4">
                      <div className="flex flex-col">
                         <span className="text-lg font-black text-slate-900 leading-none tracking-tight">{user.followers}</span>
                         <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest font-mono">Linked_Nodes</span>
                      </div>
                      <div className="w-px h-6 bg-slate-100" />
                      <div className="flex flex-col">
                         <span className="text-lg font-black text-slate-900 leading-none tracking-tight">{user.trustTier || 'Alpha'}</span>
                ...
                <div className="mt-auto">
                   {!isMe && (
                     <button 
                       ...
                       className={`w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-sm active:scale-95 disabled:opacity-50 ${isFollowing ? 'bg-white border border-slate-200 text-slate-500 hover:text-rose-500 hover:border-rose-200' : 'bg-slate-900 text-white hover:bg-amber-500 hover:text-white shadow-lg'}`}
                     >
      ...
      {verifiedUsers.length === 0 && (
        <div className="py-32 flex flex-col items-center justify-center text-center opacity-40 bg-slate-50/50 rounded-[3rem] border border-dashed border-slate-200">
           <div className="w-24 h-24 bg-white rounded-[2.5rem] flex items-center justify-center mb-6 text-slate-300 shadow-sm border border-slate-100">
              <ICONS.Verified />
           </div>
           <h3 className="text-2xl font-black uppercase tracking-widest italic text-slate-900">Index_Empty</h3>
           <p className="text-[10px] font-black uppercase tracking-[0.3em] font-mono mt-2 text-slate-400">
