...
export const LiveBroadcastOverlay: React.FC<LiveBroadcastOverlayProps> = ({ 
  ...
  // 1. Initialize hardware mirror immediately on mount
  useEffect(() => {
  ...
  // 2. Setup signaling once the stream is active in Firestore
  useEffect(() => {
    ...
    <div className="fixed inset-0 z-[2500] bg-slate-950 flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0">
        <video 
          ...
          className={`w-full h-full object-cover transition-all duration-1000 mirror ${isLive ? 'opacity-90 blur-0 scale-100' : 'opacity-30 blur-2xl scale-110'}`} 
        />
        <div className={`absolute inset-0 transition-all duration-1000 ${isLive ? 'bg-gradient-to-t from-black/80 via-transparent to-black/40' : 'bg-slate-950/80'}`} />
      </div>

      {/* SETUP STAGE: Mandatory Title Entry */}
      {!isLive && (
        <div className="relative z-[2600] w-full max-w-xl p-8 animate-in fade-in slide-in-from-bottom-10 duration-700">
           <div className="bg-white/10 backdrop-blur-3xl rounded-[3.5rem] p-10 md:p-14 border border-white/20 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.5)] flex flex-col items-center text-center">
              
              <div className="w-24 h-24 bg-rose-600 rounded-[2.5rem] flex items-center justify-center mb-10 shadow-[0_20px_50px_rgba(225,29,72,0.4)] relative">
                <div className="absolute inset-0 rounded-[2.5rem] bg-rose-500 animate-ping opacity-20" />
                <svg className="w-10 h-10 text-white relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
              </div>

              <div className="mb-12">
                <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter uppercase italic mb-3">Broadcast Prep</h2>
                <p className="text-[10px] font-black text-rose-500 uppercase tracking-[0.4em] font-mono">Establish Neural Uplink Identity</p>
              </div>

              <div className="w-full space-y-8">
                <div className="space-y-4 text-left">
                  <label className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] font-mono ml-6">Transmission_Signal_Title</label>
                  <input 
                    autoFocus
                    type="text" 
                    maxLength={50}
                    placeholder="Enter Signal Label..." 
                    value={streamTitle}
                    onChange={(e) => setStreamTitle(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-[2rem] px-10 py-7 text-white font-black outline-none text-center focus:ring-4 focus:ring-rose-500/30 focus:bg-white/10 transition-all placeholder:text-white/10 text-2xl italic tracking-tight"
                  />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col items-center gap-2 p-4 bg-white/5 rounded-2xl border border-white/5">
                    <div className={`w-2 h-2 rounded-full ${hwStatus === 'ready' ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : 'bg-rose-500'}`} />
                ...
                <div className="pt-4 flex flex-col gap-4">
                  <button 
                    ...
                    className="w-full py-7 bg-rose-600 text-white rounded-[2rem] font-black text-[12px] uppercase tracking-[0.4em] shadow-2xl shadow-rose-900/40 active:scale-95 transition-all disabled:opacity-30 disabled:grayscale group"
                  >
                    {isInitializing ? (
                      ...
                      <span className="group-hover:tracking-[0.6em] transition-all">Go_Live_Now</span>
                    )}
                  </button>
                  <button 
                    onClick={onEnd} 
                    className="py-4 text-white/30 font-black uppercase text-[10px] tracking-widest hover:text-white transition-colors italic"
                  >
                    Abort_Broadcast
                  </button>
                </div>
      ...
      {/* LIVE STAGE: HUD & Controls */}
      {isLive && (
        <div className="relative z-[2600] flex flex-col h-full w-full p-8 md:p-12 pointer-events-none animate-in fade-in duration-1000">
           ...
           <div className="mt-auto flex items-end justify-between gap-8">
              <div className="flex items-center gap-8">
                 ...
                 <div className="text-left">
                    <p className="text-[12px] font-black text-rose-500 uppercase tracking-[0.4em] font-mono mb-3 italic">Broadcast_Identity</p>
                    ...
                    <div className="flex gap-4 mt-4">
                      ...
                      <span className="text-[10px] font-black text-white/40 uppercase tracking-widest font-mono">ID: {userData.username}</span>
                    </div>
                 </div>
              </div>
