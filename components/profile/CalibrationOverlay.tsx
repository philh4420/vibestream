    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-3xl rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.4)] overflow-hidden flex flex-col h-[90vh] border border-white/20 animate-in zoom-in-95 duration-500">
        ...
        <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-10 scroll-container">
          {activeSubTab === 'core' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-2 group">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 font-mono group-focus-within:text-indigo-500 transition-colors">Identity_Bio</label>
                <textarea 
                  value={form.bio} 
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="flex items-center gap-10 bg-slate-50/50 p-8 rounded-[3rem] border border-slate-100">
                ...
                <div className="flex-1 space-y-3">
                   ...
                   <p className="text-xs text-slate-400 font-medium leading-relaxed max-w-sm">Neural identity visuals are broadcasted across the grid in high fidelity.</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-6">
                 <div className="flex justify-between items-center border-b border-slate-50 pb-4">
                    ...
                    <button onClick={addSocialLink} className="px-5 py-2.5 bg-indigo-50 text-indigo-600 rounded-xl text-[9px] font-black uppercase tracking-widest border border-indigo-100">+ Link Node</button>
                 </div>
                 <div className="space-y-4">
                    {form.socialLinks.map((link, idx) => (
                      <div key={idx} className="flex gap-4 p-5 bg-slate-50 rounded-[1.8rem] border border-slate-100 animate-in slide-in-from-top-2">
                        <select 
                          value={link.platform} 
                          onChange={(e) => updateSocialLink(idx, 'platform', e.target.value)}
                          className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold"
                        >
                        ...
                        />
                        <button onClick={() => removeSocialLink(idx)} className="p-3 text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7" /></svg></button>
                      </div>
                    ))}
                 </div>
              </div>

              <InputField label="Identity_Tags" value={form.tags} onChange={(e: any) => setForm({...form, tags: e.target.value})} placeholder="Designer, Architect..." />
              <InputField label="Passions_Grid (Hobbies)" value={form.hobbies} onChange={(e: any) => setForm({...form, hobbies: e.target.value})} placeholder="Photography, Deep Learning..." />
            </div>
          )}
          ...
          {activeSubTab === 'chronology' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
               <div className="flex justify-between items-center border-b border-slate-50 pb-4">
                  ...
                  <button onClick={addLifeEvent} className="px-5 py-2.5 bg-indigo-50 text-indigo-600 rounded-xl text-[9px] font-black uppercase tracking-widest border border-indigo-100">+ Marker</button>
               </div>
               <div className="space-y-6">
                  {form.lifeEvents.map((event, idx) => (
                    <div key={event.id} className="p-8 bg-slate-50 rounded-[2.2rem] border border-slate-100 relative group">
                       ...
                       <button onClick={() => removeLifeEvent(idx)} className="absolute -top-3 -right-3 p-3 bg-white text-rose-500 rounded-full shadow-lg border border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg></button>
                    </div>
                  ))}
               </div>
            </div>
        ...
        <div className="shrink-0 p-8 md:p-12 pt-6 flex gap-6 bg-slate-50/50 border-t border-slate-100">
          <button onClick={onClose} className="flex-1 py-5 bg-white text-slate-400 border border-slate-200 rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] active:scale-95">Abort_Sync</button>
          <button onClick={handleCommit} className="flex-1 py-5 bg-indigo-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] shadow-2xl active:scale-95">Commit_Calibration</button>
        </div>
      </div>
    </div>
  );
