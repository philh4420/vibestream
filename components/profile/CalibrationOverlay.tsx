...
const SelectField = ({ label, value, onChange, options, icon: Icon }: any) => (
  <div className="space-y-2 group">
    ...
    <select 
      value={value} 
      onChange={onChange} 
      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all appearance-none cursor-pointer"
    >
...
export const CalibrationOverlay: React.FC<CalibrationOverlayProps> = ({ userData, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-[90vh] border border-white/20 animate-in zoom-in-95 duration-500">
        ...
        <div className="shrink-0 p-8 md:p-12 pb-0 flex justify-between items-start bg-gradient-to-b from-slate-50/50 to-transparent">
           <div>
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] font-mono">Neural Handshake Protocol v2.6.9</p>
           </div>
           <button onClick={onClose} className="p-5 bg-slate-100 hover:bg-slate-200 rounded-2xl transition-all text-slate-500 active:scale-90">
        ...
        <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-10 scroll-container">
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="flex items-center gap-10 bg-slate-50/50 p-8 rounded-[3rem] border border-slate-100">
                ...
                <div className="flex-1 space-y-3">
                   ...
                   <p className="text-xs text-slate-400 font-medium leading-relaxed max-w-sm">Identity visuals are broadcasted across the VibeStream Grid in high fidelity.</p>
                </div>
              </div>
            </div>
          )}
          ...
          {activeSubTab === 'logs' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
               <div className="flex justify-between items-center border-b border-slate-50 pb-4">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] font-mono">System_Identity_Logs</h4>
                  <button onClick={() => setForm(prev => ({...prev, lifeEvents: [...prev.lifeEvents, { id: Math.random().toString(36).substring(7), title: '', date: '', icon: '🌟' }]}))} className="px-5 py-2.5 bg-indigo-50 text-indigo-600 rounded-xl text-[9px] font-black uppercase tracking-widest border border-indigo-100 hover:bg-indigo-100 transition-colors">+ Add Log Entry</button>
               </div>
               <div className="space-y-6">
                  {form.lifeEvents.map((event, idx) => (
                    <div key={event.id} className="p-8 bg-slate-50 rounded-[2.2rem] border border-slate-100 space-y-6 relative group">
        ...
        <div className="shrink-0 p-8 md:p-12 pt-6 flex gap-6 bg-slate-50/50 border-t border-slate-100">
          <button onClick={onClose} className="flex-1 py-6 bg-white text-slate-400 border border-slate-200 rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] active:scale-95 hover:bg-slate-50 transition-all">Abort_Sync</button>
          <button onClick={handleCommit} className="flex-1 py-6 bg-indigo-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] shadow-2xl active:scale-95 hover:bg-indigo-700 transition-all">Commit_Calibration</button>
        </div>
      </div>
    </div>
  );
