    ...
    <div className="glass-panel rounded-[2.5rem] p-8 mb-8">
      ...
      {mediaPosts.length === 0 ? (
        <div className="py-20 text-center bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
           <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">No Visual Signals Recorded</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {mediaPosts.map(post => (
            <div key={post.id} className="aspect-square rounded-2xl overflow-hidden border-precision bg-slate-100 group cursor-pointer relative">
               <img 
               ...
               <div className="absolute inset-0 bg-indigo-600/0 group-hover:bg-indigo-600/40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                  <span className="text-white font-black text-[10px] uppercase tracking-widest">View Post</span>
               </div>
            </div>
          ))}
        </div>
