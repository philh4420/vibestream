...
const ProtocolCard: React.FC<ProtocolCardProps> = ({ label, route, isActive, onToggle }) => {
  return (
    <div className={`relative p-6 rounded-[2.5rem] border transition-all duration-500 flex flex-col justify-between min-h-[220px] overflow-hidden group ${
      isActive 
        ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 shadow-xl shadow-slate-200/50 dark:shadow-none' 
      ...
      <div className="relative z-10 flex justify-between items-start">
        <div className={`w-14 h-14 rounded-[1.5rem] flex items-center justify-center transition-all duration-500 ${
             isActive 
               ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg shadow-indigo-500/20 dark:shadow-none' 
               : 'bg-white dark:bg-slate-700 border border-slate-100 dark:border-slate-600 text-slate-300 dark:text-slate-500'
           }`}>
             <ICONS.Settings />
        </div>
        
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border ${isActive ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800' : 'bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-600'}`}>
           <div className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${isActive ? 'bg-emerald-500 shadow-[0_0_8px_#10b981] animate-pulse' : 'bg-slate-400 dark:bg-slate-500'}`} />
      ...
      <div className="relative z-10 mt-6">
         <p className="text-[9px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-[0.3em] font-mono mb-2">
           LAYER_{route.toUpperCase().slice(0,3)}
         </p>
         <h4 className={`text-2xl font-black uppercase tracking-tighter italic leading-none transition-colors ${isActive ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>
           {label.replace('_', ' ')}
         </h4>
      </div>
      ...
      <div className="mt-auto pt-6 relative z-10">
        <button 
          ...
          className={`w-full py-4 rounded-[1.8rem] flex items-center justify-between px-2 transition-all duration-500 ${
            isActive ? 'bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800' : 'bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600'
          }`}
        >
          <span className={`text-[9px] font-black uppercase tracking-widest font-mono ml-4 ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>
            {isActive ? 'ACTIVE' : 'INACTIVE'}
          </span>
          
          <div className={`h-10 w-16 rounded-[1.4rem] p-1 flex items-center transition-all duration-500 ${isActive ? 'bg-indigo-600 dark:bg-indigo-500 shadow-lg' : 'bg-slate-300 dark:bg-slate-600'}`}>
...
export const AdminProtocols: React.FC<AdminProtocolsProps> = ({ systemSettings, handleToggleFeature }) => {
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {/* Hero Header */}
      <div className="bg-slate-950 dark:bg-black rounded-[3rem] p-10 md:p-14 text-white shadow-2xl relative overflow-hidden border border-white/5 dark:border-slate-800">
